...
3. Enable code signing for `com.adplay.app` (App Store distribution). After turning on **Sign in with Apple** on the App ID, refresh / regenerate that profile so the IPA includes `com.apple.developer.applesignin`.
